...
<style scoped>
.board-page{--bg:#f3efe6;--panel:#fffdf8;--ink:#171717;--muted:#706d66;--line:#242424;--accent:#d9ff55;--pink:#f5b6d1;min-height:100vh;min-height:100dvh;background:var(--bg);color:var(--ink);font-family:Inter,"Microsoft YaHei",sans-serif;transition:.2s;background-color:.2s}
.board-page.dark{--bg:#111310;--panel:#1d211b;--ink:#f5f1e9;--muted:#aaa99f;--line:#d9ded0;--accent:#c8ef4d;--pink:#d98eae}
.board-topbar{height:68px;padding:0 clamp(16px,4vw,56px);border-bottom:2px solid var(--line);display:grid;grid-template-columns:1fr auto 1fr;align-items:center;background:var(--panel);position:sticky;top:0;z-index:20}
...
@media(max-width:720px){.game-board{box-shadow:none}.gomoku-board .board-piece{inset:-6%;width:112%;height:112%}.board-main{padding:8px}.setup-panel{box-shadow:none}}
.board-result-overlay{position:fixed;z-index:60;inset:0;display:grid;place-items:center;padding:18px;background:rgba(3,8,31,.48);backdrop-filter:blur(7px)}.board-result-overlay .result-panel{width:min(420px,94vw);margin:0;border:1px solid rgba(224,241,255,.52);background:linear-gradient(145deg,rgba(32,48,105,.96),rgba(8,16,52,.98));color:#fff;box-shadow:0 25px 90px rgba(0,0,30,.55);padding:32px}.board-result-overlay .result-panel>strong{font-family:'Noto Serif SC','Songti SC',serif;font-size:clamp(28px,5vw,40px)}.board-result-overlay .result-panel>span{display:block;margin-top:8px;color:rgba(239,244,255,.68)}.board-result-overlay .result-panel button{border:1px solid rgba(255,255,255,.55);border-radius:999px;background:rgba(125,197,255,.16);color:#fff;box-shadow:none}.board-result-overlay .result-panel button:hover{background:#fff;color:#101b47}
.board-brand{min-width:0;white-space:nowrap}.board-brand span{overflow:hidden}.board-brand span img{display:block;width:82%;height:82%;object-fit:contain}.board-brand strong{white-space:nowrap;font-size:clamp(12px,2.1vw,16px)}
...
.setup-panel h1{font-size:clamp(26px,3vw,40px);white-space:nowrap;word-break:keep-all;overflow-wrap:normal}
.board-main{width:100%;max-width:none}
@media (min-width:721px){
  .board-page{height:100dvh;overflow:hidden}
  .board-main{height:calc(100dvh - 66px);grid-template-columns:minmax(220px,270px) minmax(0,1fr)}
